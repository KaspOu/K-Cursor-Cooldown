local L = LibStub("AceLocale-3.0"):NewLocale("CC", "enUS")
if not L then return end

L["Add Spell"] = "Add Spell"
L["All specified colors are vertex colors! (see wowwiki.com)"] = "All specified colors are vertex colors! (see wowwiki.com)"
L["All unlockable modules are disabled!"] = "All unlockable modules are disabled!"
L["Attach to cursor"] = "Attach to cursor"
L["Auto arrange icons"] = "Auto arrange icons"
L["Background"] = "Background"
L["Bar"] = "Bar"
L["Bar position saved!"] = "Bar position saved!"
L["Border"] = "Border"
L["Bottom"] = "Bottom"
L["Bottom has to be a number!"] = "Bottom has to be a number!"
L["Cast color"] = "Cast color"
L["Casttime"] = "Casttime"
L["Color"] = "Color"
L["Colors"] = "Colors"
L["Display"] = "Display"
L["Enabled"] = "Enabled"
L["Font"] = "Font"
L["Font Size"] = "Font Size"
L["General"] = "General"
L["Hide default castbar"] = "Hide default castbar"
L["Horizontal Offset"] = "Horizontal Offset"
L["Icon size"] = "Icon size"
L["Invert"] = "Invert (show only spells not on cooldown)"
L["Latency color"] = "Latency color"
L["Left"] = "Left"
L["Left has to be a number!"] = "Left has to be a number!"
L["Midnight: GCD is not available in combat"] = "Midnight: GCD is not available in combat"
L["Miscellaneous"] = "Miscellaneous"
L["Position"] = "Position"
L["Primary"] = "Primary"
L["Radius"] = "Radius"
L["Remove Spell"] = "Remove Spell"
L["Restore defaults"] = "Restore defaults"
L["Reverse channeling"] = "Reverse channeling"
L["Right Click"] = "Right Click"
L["Ring"] = "Ring"
L["Rotate"] = "Rotate"
L["Secondary"] = "Secondary"
L["Show spark only"] = "Show spark only"
L["Spark"] = "Spark"
L["Spell Text"] = "Spell Text"
L["Spellcds"] = "Spellcds"
L["Spells"] = "Spells"
L["Talent Specs"] = "Talent Specs"
L["Text"] = "Text"
L["Texture"] = "Texture"
L["to lock the cooldown bar and save its position"] = "to lock the cooldown bar and save its position"
L["Thickness"] = "Thickness"
L["Unlock"] = "Unlock"
L["Vertical Offset"] = "Vertical Offset"
L["Visibility"] = "Visibility"
L["GCD / Casting only"] = "GCD / Casting only"
L["In combat"] = "In combat"
L["In raid"] = "In raid"
L["In party/raid"] = "In party/raid"
L["Always"] = "Always"
L["Width"] = "Width"
L["x-Offset"] = "x-Offset"
L["y-Offset"] = "y-Offset"
L["Filter"] = "Filter"
L["Role filter"] = "Role filter"
L["When enabled, the addon is hidden when your current spec's role is disabled below."] = "When enabled, the addon is hidden when your current spec's role is disabled below."
L["Enable role filter"] = "Enable role filter"
