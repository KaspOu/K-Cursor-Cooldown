...
function module:OnEnable()
	self:ApplyOptions()
	-- self:RegisterUnitEvent("UNIT_SPELLCAST_SENT", "player") -- UNIT_SPELLCAST_SUCCEEDED?
...
function module:UNIT_SPELLCAST_START(_, unit, action)
	-- if not UnitIsPlayer(unit) then return end
	local text, spell
	spell, text, _, castStartTime, castEndTime, _, _, _, _ = addon.UnitCastingInfo(unit)
...
function module:UNIT_SPELLCAST_DELAYED(_, unit)
	-- if not UnitIsPlayer(unit) then return end
	_, _, _, _, castStartTime, castEndTime = addon.UnitCastingInfo(unit)
	castDuration = castEndTime and castEndTime - castStartTime or 0
end
